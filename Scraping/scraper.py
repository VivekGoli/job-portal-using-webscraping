...
def cisco_parser(url):
    ...
    for i in range(len(links)):
        try:
            ...
            job_description = link_soup.find("div", class_="job_description").text
    ...
    #jobs_dataframe.to_csv("Scraping\cisco.csv")
    print("Cisco ended")
...
def google_parser(url):
    ...
    def google_inner_parser(url):
        ...
        for i in range(len(links)):
            try:
                ...
                lists = link_soup.find("div", class_="KwJkGe").find_all("ul")
                skills = ""
                for list in lists:
                    for item in list.find("li"):
                        skills += item.text + ".\n"

                list = link_soup.find("div", class_="BDNOWe").find("ul")
                responsibilities = ""
                for item in list.find("li"):
                    responsibilities += item.text + ".\n"
    ...
    #jobs_dataframe.to_csv("Scraping\google.csv")
    print("Google ended")
    return jobs_dataframe
...
def deloitte_parser(url):
    ...
    jobs_dataframe = pd.DataFrame(jobs, columns=["company", "title", "location", "description", "link"])
    #jobs_dataframe.to_csv("Scraping\deloitte.csv")
...
def ey_parser(url):
    ...
    def ey_inner_parser(url):
        ...
        for i in range(len(links)):
            try:
                ...
                for para in paras:
                    job_description += ". " + para.text.strip()
                uls = span.find_all("ul")
                for ul in uls:
    ...
    jobs_dataframe = pd.DataFrame(jobs, columns=["company", "title", "location", "description", "link"])
    #jobs_dataframe.to_csv("Scraping\ey.csv")
    print("EY ended")
    return jobs_dataframe
...
def sap_parser(urls):
    ...
    jobs_dataframe = pd.DataFrame(jobs, columns=["company", "title", "location", "description", "link"])
    #jobs_dataframe.to_csv("Scraping\sap.csv")
    print("SAP ended")
    return jobs_dataframe
...
def hsbc_parser(url):
    ...
    def hsbc_inner_parser(url):
        ...
        try:
            ...
            for i in range(len(links)):
                ...
                jobdescription = ""
                divs = link_soup.find_all("div", class_="article__content__view__field__value")
                for div in divs:
    ...
    jobs_dataframe = pd.DataFrame(jobs, columns=["company", "title", "location", "description", "link"])
    #jobs_dataframe.to_csv("Scraping\hsbc.csv")
    print("HSBC ended")
    return jobs_dataframe
...
def electronicarts_parser(url):
    ...
    for i in range(len(links)):
        try:
            ...
            jobdescription = ""
            for div in link_soup.find_all("div", class_="job-details"):
            ...
            pass
    jobs_dataframe = pd.DataFrame(jobs_list, columns=["company", "title", "location", "description", "link"])
    #jobs_dataframe.to_csv("Scraping\electronic_arts.csv")
    print("EA ended")
    return jobs_dataframe
...
def bank_of_america_parser(urls):
    ...
    def bankofamerica_inner_parser(url):
        ...
        for i in range(len(links)):
            try:
                ...
                for list in div.find_all("ul"):
                    for item in list.find("li"):
                        jobdescription += item.text.strip() + ".\n"
                jobdescription += "\n"
                for para in div.find_all("p"):
                    jobdescription += para.text.strip()
                details.append(jobdescription)
                details.append(links[i])
    ...
    jobs_dataframe = pd.DataFrame(jobs, columns=["company", "title", "location", "description", "link"])
    #jobs_dataframe.to_csv("Scraping\\bank_of_america.csv")
...
def massmutual_parser(urls):
    ...
    jobs_dataframe = pd.DataFrame(jobs, columns=["company", "title", "location", "description", "link"])
    #jobs_dataframe.to_csv("Scraping\massmutual.csv")
...
def hitachi_parser(url):
    ...
    for thread in threads:
        ...
        jobs = jobs + thread.result
    jobs_dataframe = pd.DataFrame(jobs, columns=["company", "title", "location", "description", "link"])
    #jobs_dataframe.to_csv("Scraping\hitachi.csv")
    print("Hitachi ended")
    return jobs_dataframe
...
if __name__=="__main__":
    ...
    dataframe.to_csv("Scraping\\alljobs.csv", index=False)
